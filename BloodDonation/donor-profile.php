<html>




<head>
    
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
    
<title>Donar_Profile</title>
    
    
     <link rel="stylesheet" href="bootstrap/css/bootstrap.css">
    <script type="text/javascript" src="bootstrap/js/jquery-1.9.1.js"></script>
    <script type="text/javascript" src="bootstrap/js/bootstrap.js"></script>
    

    <style>
        
        #head
        {
            margin-top: 100px;
            text-shadow: 3px 3px 3px gray;
            font-size: 40px;
           font-family: Calibri;
            font-weight: bold;
            text-decoration: underline;
        }
    
        .mrg
        {
            margin-top: 50px;
            font-family: calibri;
            font-weight: bold;
        }
        
        input[type="text"]
        {
          box-shadow: 3px 3px 3px black;
        }
        
         button
        {
          box-shadow: 3px 3px 3px black;
        }
    
        img
        {
            box-shadow: 3px 3px 3px black;
        }
        
        .bg
        {
            
               left: 0;
               top: 0;
          width: 100%;
           
            background-size: cover;
        background-repeat: no-repeat;
            background-image: url(pics/bg4.jpg);
        }
    
             
        .trans
        {
             background: rgba(120,110,100,.5);
            
        }
    </style>

     <script type="text/javascript">
        
        function showpic(file)
        {
              if (file.files && file.files[0])
		 {
            var reader = new FileReader();
            reader.onload = function (e) {
                $('#pic').attr('src', e.target.result);
                 
              
            }
            reader.readAsDataURL(file.files[0]);
        }
        
        }
        
         
        $(document).ready(function(){
            
            $("#search").click(function(){
                
               $uid = $("#uid").val();
                
                $.getJSON("fetch-donor-profile.php?uid="+$uid,function(jasonArray){
                    
                    
                    $("#fname").val(jasonArray[0].fname);
                    $("#lname").val(jasonArray[0].lname);
                    $("#age").val(jasonArray[0].age);
                    $("#bloodgroup").val(jasonArray[0].bloodgroup);
                    
                    $gender=jasonArray[0].gender;
                    if($gender=="male")
                        {
                            $("#male").prop('checked',true);
                        }
                    else
                    $("#female").prop('checked',true);
                    
                    $("#address").val(jasonArray[0].address);
                    $("#city").val(jasonArray[0].city);
                    $("#mobileno").val(jasonArray[0].mobileno);
                    $("#blooddonated").val(jasonArray[0].blooddonated);
                    $("#medicalhistory").val(jasonArray[0].medicalhistory);
                    
                    $("#pic").prop("src","uploads/"+jasonArray[0].profilepic);
                    
                    $("#hdn").val(jasonArray[0].profilepic);
                    
                    
                });
            
            });  
        
        }); 
         
        
        
        </script>
    
    
    </head>





<body class="bg">
     <container>
           
           
            <form name="frm" action="donor-profile-process.php" method="post" enctype="multipart/form-data">
 
  <input type="hidden" id="hdn" name="hdn">
   
   
    <div class="row">
       <div class="col-md-12">
           
           <nav class="navbar fixed-top navbar-expand-lg navbar-dark " style="background-color:#801a00;">
 <a class="navbar-brand " href="#">
    <img src="pics/donateblood.png" width="50" height="50" class="d-inline-block align-items-center" alt="">
    Blood Donation
  </a>
  <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbarSupportedContent" aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation">
    <span class="navbar-toggler-icon"></span>
  </button>

  <div class="collapse navbar-collapse" id="navbarSupportedContent">
    <ul class="navbar-nav mr-auto ">
      <li class="nav-item active">
        <a class="nav-link text-white " href="#">Home <span class="sr-only">(current)</span></a>
      </li>
       <li class="nav-item active">
        <a class="nav-link text-white" href="donor-profile.php">Donar<span class="sr-only">(current)</span></a>
      </li>
     
      <li class="nav-item active">
        <a class="nav-link text-white" href="ngo-registeration.html">N.G.O<span class="sr-only">(current)</span></a> 
      </li>
      
       <li class="nav-item active">
        <a class="nav-link text-white" href="needy-profile.php">Needy<span class="sr-only">(current)</span></a> 
      </li>
       </ul>
    
    
      <input class="form-control col-2 mr-sm-2" type="search" name="uid" id="uid" placeholder="Donor ID" aria-label="Search">
      <button class="btn btn-outline-light my-2 my-sm-0 " type="button" name="btn" id="search" value="search" >Search</button>
    
  </div>
</nav>
           
           
       </div>
       
       
        
    </div>
    
    
    <center class="trans">
<!--      heading   -->
        
         <div id="head">Donor Profile</div>
        
        
        
<!--            profile picture   -->
        <div class="text-center" style="margin-top:30px; ">
  <img src="pics/donnardp.png" id="pic" height="220px" width="200px" class="rounded" style="border:1px solid #801a00;" >
</div>
<!--    browse pic -->
       
          
        <div class="custom-file" style="width:100px;">
  <input type="file" class="custom-file-input" id="profilepic" name="profilepic" onchange="showpic(this);">
  <label class="custom-file-label" for="customFile"></label>
</div>
        
<!--     name -->
  <div class="form-row" style="margin-top:50px;">
     <div class="col-3 offset-3">
     
<!--   first name-->
         <input type="text" class="form-control" id="fname" name="fname" placeholder="First name">
    </div>
    
    <div class="col-3 ">
     
<!--    last name -->
      <input type="text" class="form-control" id="lname" name="lname" placeholder="Last name">
    </div>
  </div>
  
 
   <div class="mrg form-row align-items-center">
   
    <!--  age -->
   <div class="col-0 offset-4">
         <label for="age" >Age :</label>
    </div>
    <div class="col-1 "  >
      <input type="text" class="form-control" id="age" name="age">
    </div>
   
       <!--  Blood Group -->
       <div class="col-0 offset-1">
         <label for="bloodgroup" >Blood Group :</label>
    </div>
    <div class="col-1 "  >
      <input type="text" class="form-control" id="bloodgroup" name="bloodgroup">
    </div>
        </div>
        
        
        
<!--        gender -->
        <div class="mrg">
        <label class="form-check-label">Gender :</label>

        <div class="form-check form-check-inline">
  <input class="form-check-input" type="radio" name="gender" id="male" value="male">
  <label class="form-check-label" for="male"  style="font-weight:400;">Male</label>
</div>
<div class="form-check form-check-inline">
  <input class="form-check-input" type="radio" name="gender" id="female" value="female">
  <label class="form-check-label" for="female" style="font-weight:400;">Female</label>
</div>

 </div>       
        
        
        
     <br><hr class="col-6" style="height:1px; background-color:#801a00;">
      
      
<!--      address-->
    <div class="form-group row mrg">
    <label for="address" class="col-2 offset-3 col-form-label">Address :</label>
    <div class="col-2">
       <textarea rows="3" cols="40" id="address" name="address" placeholder="address"></textarea>
       
        </div>
  </div> 
     
      
       
<!--        city-->
   <div class="form-group row mrg">
    <label for="city" class="col-2 offset-4 col-form-label">City :</label>
    <div class="col-2">
      <input type="text" class="form-control" id="city" name="city" placeholder="city">
    </div>
  </div>
  
<!--  mobile-->
  <div class="form-group row mrg">
    <label for="mobileno" class="col-2 offset-4 col-form-label">Mobile Number :</label>
    <div class="col-2">
      <input type="text" class="form-control" id="mobileno" name="mobileno" placeholder="mobile number">
    </div>
  </div>
   
<!--   no.of times blood donated-->
     <div class="form-group row mrg">
    <label for="blooddonated" class="col-3 offset-3 col-form-label">No. of Times Blood Donated :</label>
    <div class="col-2">
      <input type="text" class="form-control" id="blooddonated" name="blooddonated" placeholder="number">
    </div>
  </div>
     
       <!--      Medical history-->
    <div class="form-group row mrg">
    <label for="medicalhistory" class="col-2 offset-3 col-form-label">Medical History :</label>
    <div class="col-2">
       <textarea  rows="3" cols="40" id="medicalhistory" name="medicalhistory" placeholder="medical history"></textarea>
       
        </div>
  </div> 
       
           <br><hr class="col-6" style="height:1px; background-color:#801a00;"><br>
           
                        <button type="submit" class="btn btn-primary" name="btn" id="save" value="save">Save</button>
                        <button type="submit" class="btn btn-dark" name="btn" id="update" value="update">Update</button>
                       

  

            <br><br><hr class="col-6" style="height:1px; background-color:#801a00;"><br>
        
    </center>
    </form>
    
    
    </container>
       
</body>



</html>